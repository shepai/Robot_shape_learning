...
class task1(task):
    ...
    def solve(self,env):
        ...
        for i in range(len(env.block_ids)): #sort the ids in order so we take the furthest ones first
            target_pos, _ = env.getBasePositionAndOrientation(env.block_ids[i])
            #insertion sort 
        ...
        for i in range(len(env.block_ids)):
            cube_pos, _ = env.getBasePositionAndOrientation(env.block_ids[in_order[i]]) #find id
...
class task2(task):
    ...
    def solve(self,env):
        ...
        for i in range(len(env.block_ids)):
            ...
            cube_pos, _ = env.getBasePositionAndOrientation(env.block_ids[i]) #find id
            cube_pos=list(cube_pos)
            cube_pos[2]+=0.08
            env.move_gripper_to(cube_pos) #move to just above it
            env.step(10)
...
class task5(task):
    ...
    def solve(self,env,p):
        ...
        cube_pos, _ = env.getBasePositionAndOrientation(ball) #find id
        temp_loc, _ = env.getBasePositionAndOrientation(block) #find id
        ...
        for x in range(size): #loop through potential grid
            for y in range(size):
                ...
                if type(relatedx)!=type(None) and type(relatedy)!=type(None):
                    ...
                    if len(overlap) > 0:
                        # cell occupied
                        visual_data = env.getVisualShapeData(env.block_ids[c])
        ...
        for yi, xi in zip(y, x):
            ...
            for i in range(len(targets)):
                if targets[i][4]==yi and targets[i][5]==xi: #put in corret order
                    ...
                    id_=targets[i][3]
                    cube_pos, _ = env.getBasePositionAndOrientation(env.block_ids[id_]) #find id
                    ids.append(cube_pos)
    ...
    def solve(self,env,p):
        ...
        for i in range(len(env.block_ids)):
            ...
            if max(colour[0:3])>highest: #if the maximum channel is less than current lowest
                ...
                highest_idx=i
        cube_pos, _ = env.getBasePositionAndOrientation(env.block_ids[highest_idx]) #find id
        cube_pos=list(cube_pos)
        cube_pos[2]+=0.08
        env.move_gripper_to(cube_pos) #move to just above it
        env.step(10)
